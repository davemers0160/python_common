...
import math
import pcbnew
from PyQt6.QtWidgets import (QApplication, QDialog, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton)
...
#------------------------------------------------------------------------------
class SpiralAction(pcbnew.ActionPlugin):
    def defaults(self):
        self.name = "Generate Spiral Footprint"
        self.category = "Footprint Generation"
        self.description = "Creates a multi-arm spiral with keepouts"
        self.show_toolbar_button = True
        
        # Optional: path to a 26x26px png icon
        self.icon_file_name = os.path.join(os.path.dirname(__file__), 'spiral.png')

    def Run(self):
        # Handle the QApplication lifecycle within KiCad
        ...
        if dialog.exec():
            ...
            self.create_spiral(params)

    def create_spiral(self, params):         
        # Parameters
...
#------------------------------------------------------------------------------
# This is the critical part for registration
SpiralAction().register()
